...
import { CalendarIcon, Check, ExternalLink, ImagePlus, PlusCircle } from "lucide-react";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
            ...
            <div className="border-t pt-4">
              ...
              <div className="space-y-2">
                {history.map((p) => (
                  ...
                  >
                    <span>
                      Package #{p.package_number} · {formatDate(p.start_date)} –{" "}
        ...
        <DialogFooter className="sm:justify-between">
          <Link
            href={`/annual-partners/${partner.id}`}
            className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"
          >
            View full profile (notes & post schedule) <ExternalLink className="h-3.5 w-3.5" />
          </Link>
